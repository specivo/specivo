...
async def wiki_page(db_session, project, admin):
    page, _ = await WikiService().create_page(
        db_session, project.id, title="Tagged Page", text="hello", author=admin
    )
    await db_session.commit()
...
class TestTagVocabulary:
    async def test_create_and_list(self, client, project, admin_token):
        ...
        resp = await client.get(
            f"/api/v1/projects/{project.key}/tags/", headers=_auth(admin_token)
        )
    ...
    async def test_delete(self, client, project, admin_token):
        ...
        resp = await client.delete(
            f"/api/v1/projects/{project.key}/tags/{tag_id}/", headers=_auth(admin_token)
        )
...
class TestIssueTagging:
    ...
    async def test_add_and_remove_one(self, client, project, issue, member_token, admin_token):
        ...
        resp = await client.get(
            f"/api/v1/issues/{issue.display_key}/tags/", headers=_auth(member_token)
        )
        assert {t["name"] for t in resp.json()} == {"review"}

        resp = await client.delete(
            f"/api/v1/issues/{issue.display_key}/tags/{tag_id}/", headers=_auth(member_token)
        )
        assert resp.status_code == 204
