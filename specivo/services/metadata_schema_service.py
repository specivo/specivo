...
class MetadataSchemaService:
    ...
    async def list_for_project(
        ...
        stmt = select(MetadataSchema).where(MetadataSchema.project_id == project_id).order_by(MetadataSchema.name.asc())
    ...
    async def update(
        ...
        if data.schema_definition is not None:
            validate_json_schema(data.schema_definition)
            schema.schema_definition = data.schema_definition
        if data.name is not None:
            schema.name = data.name
        if data.description is not None:
            schema.description = data.description
        if data.tracker_id is not None:
            schema.tracker_id = data.tracker_id
        session.add(schema)
        await session.flush()
