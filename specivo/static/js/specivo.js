...
document.addEventListener('alpine:init', function () {
    ...
    /**
     * Search-page tag filter — read-only multi-select over real tags.
     *
     * Suggests distinct tag names across all accessible projects (deduped) via
     * the global ``/api/v1/tags/search/`` endpoint. Selecting or removing a tag
     * reloads the search page with the updated set of repeated ``tag=`` params,
     * preserving the query, mode, scope, project, and page size. Unlike
     * ``tagField`` it never persists or creates tags — it only drives the URL.
     */
    Alpine.data('tagFilter', function (initial) {
        return {
    ...
    /**
     * Search FTS language + reindex control.
     *
     * Drives both the instance-wide (admin) and per-project search index
     * settings. Lets a manager pick the Postgres FTS analyzer language,
     * save it, then kick off a background reindex and watch live progress.
     *
     * Usage:
     *   x-data='ftsReindex({ baseUrl, language, allowed, reindexNeeded,
     *                        lastResult, running, canManage, isProject,
     *                        effective, instanceDefault })'
     *
     * baseUrl is the scope root, e.g. '/api/v1/admin/search' or
     * '/api/v1/projects/ACME/search'. Endpoints derived from it:
     *   PUT  {baseUrl}/fts/language/
     *   POST {baseUrl}/reindex/
     *   GET  {baseUrl}/fts/                (refresh full state)
     *   GET  {baseUrl}/reindex/status/     (poll)
     */
    Alpine.data('ftsReindex', function (initial) {
        initial = initial || {};
        return {
